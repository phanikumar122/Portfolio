...
export const TechMarquee = () => {
  ...
  return (
    <div className="relative w-full py-5 sm:py-6 overflow-hidden border-t border-b border-[var(--color-border)] bg-white/60 backdrop-blur-md z-10 select-none">
        ...
        <div className="flex animate-marquee gap-8 sm:gap-14 items-center whitespace-nowrap">
          {displayTechs.map((tech, idx) => (
            <div key={idx} className="flex items-center gap-4 sm:gap-7">
              <span className="text-xs sm:text-sm font-sans font-bold uppercase tracking-[0.2em] text-neutral-500 hover:text-[var(--color-primary)] transition-colors duration-300 cursor-default">
                {tech}
              </span>
              <span className="text-[var(--color-accent)] opacity-40 text-xs sm:text-sm font-bold select-none">
                {"//"}
              </span>
            </div>
          ))}
